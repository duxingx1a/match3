...
def crop_and_save(image: Image.Image, output_folder: str, crop_ratio: float = 0.4):
    ...
    参数:
        ...
        output_folder: 保存路径
        crop_ratio: 中间要切多大（默认 70%） 因为背景颜色不同 需要取中心确保精准
    """
    seen=set()
...
def process(img):
    ...
    block_w = w // 8 
    # 中心 40 % 区域的边界
    margin_y = int(block_h * 0.3)   # 上下各留 30 %
    margin_x = int(block_w * 0.3)   # 左右各留 30 %
    cy0 = margin_y
    cy1 = block_h - margin_y
    cx0 = margin_x
    cx1 = block_w - margin_x
    img_rgb = img_np[:, :, ::-1]
    img_r=img_rgb[:,:,0]
    img_g=img_rgb[:,:,1]
    img_b=img_rgb[:,:,2]
    ...
    for col in range(8):
        ...
        # 只取中心 40 % 区域
        roi = img_r[y0 + cy0 : y0 + cy1,
                    x0 + cx0 : x0 + cx1]

        totalsr.append(int(roi.mean()))
        idx += 1
    totalsg = []
    idx = 0
    row=0
    for col in range(8):
        ...
        # 只取中心 40 % 区域
        roi = img_g[y0 + cy0 : y0 + cy1,
                    x0 + cx0 : x0 + cx1]

        totalsg.append(int(roi.mean()))
        idx += 1
    totalsb = []
    idx = 0
    row=0
    for col in range(8):
        ...
        # 只取中心 40 % 区域
        roi = img_b[y0 + cy0 : y0 + cy1,
                    x0 + cx0 : x0 + cx1]

        totalsb.append(int(roi.mean()))
        idx += 1   
    for i in range(6):
